...
{
    public Vector2 DefaultResolution = new Vector2(1366, 768);
    ...
    private void Update()
    {
        if (componentCamera.orthographic)
        {
            ...
            componentCamera.orthographicSize = Mathf.Lerp(constantWidthSize, initialSize, 0);
        }
        else
        {
            float constantWidthFov = CalcVerticalFov(horizontalFov, componentCamera.aspect);
            componentCamera.fieldOfView = Mathf.Lerp(constantWidthFov, initialFov, 0);
        }
    }
